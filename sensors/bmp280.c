...
void bmp280_get_data(I2C_Handle *i2c, double *pres, double *temp) {
    ...
    if (I2C_transfer(*i2c, &i2cTransaction)) {
    	...
    	/*uint16_t eka = (msb << 8) | (lsb & 0xff);
    	uint8_t last = (xlsb & 0xf0) >> 4;
    	uint32_t tulos = eka << 4 | last;
    	return tulos;*/
    	uint8_t press_msb = rxBuffer[5];
    	uint8_t press_lsb = rxBuffer[4];
    	uint8_t press_xlsb = rxBuffer[3];
    	uint8_t temp_msb = rxBuffer[2];
    	uint8_t temp_lsb = rxBuffer[1];
    	uint8_t temp_xlsb = rxBuffer[0];
    	
    	
    	uint16_t eka = (press_msb << 8) | (press_lsb & 0xff);
    	uint8_t last = (press_xlsb & 0xf0) >> 4;
    	uint32_t painetulos = eka << 4 | last;
    	
    	uint16_t ekalam = (temp_msb << 8) | (temp_lsb & 0xff);
    	uint8_t lastlam = (temp_xlsb & 0xf0) >> 4;
    	uint32_t lampotulos = ekalam << 4 | lastlam;

        *temp = bmp280_temp_compensation(lampotulos);
        *pres = bmp280_convert_pres(painetulos) / 100;
        
        //Display_print0(hDisplay, 4, 3, *temp);
        //Display_print0(hDisplay, 8, 3, *pres);
        }
